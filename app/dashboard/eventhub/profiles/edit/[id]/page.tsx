...
import type { UpdateEventProfileRequest, EventProfile, EventStatus } from "@/types/eventhub/eventhub"
import { useToast } from "@/hooks/use-toast"
import { ArrowLeft, Calendar as CalendarIcon, MapPin, Clock, User } from "lucide-react"
import { AuthService } from "@/services/auth.service"

interface EditEventProfilePageProps {
  params: Promise<{
    id: string
  }>
}

export default function EditEventProfilePage({ params }: EditEventProfilePageProps) {
  ...
  const [formData, setFormData] = useState<UpdateEventProfileRequest>({
    title: "",
    description: "",
    date: "",
    start_time: "",
  ...
  useEffect(() => {
    const fetchEventProfile = async () => {
      const response = await EventHubService.getProfileById(parseInt(profileId))
      ...
      if (response.success && response.data) {
        ...
        setFormData({
          ...
          celebrant_name: response.data.celebrant_name || "",
        ...
        toast({
          variant: "destructive",
          title: "Error",
          description: response.error || "No se pudo cargar el perfil del evento.",
        })
        //router.push(`/dashboard/eventhub/${siteId}`)
      }
  ...
  const handleInputChange = (field: keyof UpdateEventProfileRequest, value: any) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await EventHubService.updateProfile(parseInt(profileId), formData)

      if (response.success) {
        toast({
          title: "Perfil actualizado",
          description: "El perfil del evento ha sido actualizado exitosamente.",
        })
        router.push(`/dashboard/eventhub`)
      } else {
        toast({
          variant: "destructive",
          title: "Error",
          description: response.error || "No se pudo actualizar el perfil del evento.",
        })
      }
    } catch (error) {
      toast({
        variant: "destructive",
        title: "Error",
        description: "Ocurrió un error al actualizar el perfil del evento.",
      })
    } finally {
  ...
  return (
    <AdminLayout
      ...
      currentPath={`/dashboard/eventhub/profiles/edit/${profileId}`}
    >
      <div className="p-6">
        <div className="mb-6">
        ...
        <Card className={`${secondBackgroundColor} ${principalText} max-w-4xl mx-auto`}>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              Editar Perfil de Evento
            </CardTitle>
            <CardDescription>
              Actualiza la información del perfil de evento
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {/* Token público (readonly) */}
              {eventProfile?.public_token && (
                <div className="space-y-4">
                  <h3 className="text-lg font-semibold">Token Público</h3>
                  <div className="p-4 bg-muted rounded-lg">
                    <p className="text-sm font-medium mb-1">Token Público (Solo lectura)</p>
